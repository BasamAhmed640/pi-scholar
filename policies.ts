...
- Keep explanations complete and connected. For central equations use notes.lesson.keyEquations={id,title,latex,symbols:[{symbol,definition}],assumptions,meaning,sourcePages} entries and place each own-line [[scholar-equation:ID]] beside its explanation. Scholar creates an expanded [!note] Key equation callout with displayed mathematics and "symbol → definition" entries separated by semicolons. Explicitly say when an identity has no additional assumptions; do not invent restrictions. Leave intermediate algebra and ordinary inline mathematics in prose. Preserve the exact mathematics and do not invent an equation to fill a template.
...
- Save a sourceCoverage checklist, derived from those pages rather than the objective labels alone. Include essential definitions, derivations, central equations, assumptions, worked examples/counterexamples and useful figures with stable IDs, descriptions, PDF sourcePages and exact objective strings. A definition or a result alone does not replace a derivation or the counterexample that limits it. Plan first; before completion map every item to its lessonId and an exact explanatory evidence passage from the saved text. Equation items also name the rendered equationId; figure items name the embedded snapshotId. This checklist is checked independently against the original source.
- Save the plan before drafting a long lesson: action=notes with top-level objectives and sourceCoverage (without lesson evidence yet). figureReviews, objectiveChecks, synthesis and keyPoints may each be saved independently; do not regenerate prose just to update metadata. Top-level objectives declare the section scope; lesson.objectives only associates that unit with already-declared objectives. Use coverageUpdates to change only the lesson/equation/figure pointers or exact excerpts for existing source item IDs. Copy excerpts from status with lessonId; a paraphrase is not an exact excerpt. A rejected notes call saves none of its fields unless the result explicitly says "Draft saved": correct the named fields without resending unrelated prose. Use action=status to inspect persisted work; do not use outline to repair Learn notes.
- During initial source preparation, before the first practice or mastery question, read and view every active-section page. Issue independent source calls together in one message — up to four \`read\`/\`view\` calls for different pages at a time; the harness runs them concurrently and each still records its own durable receipt. Then issue the crops for the pages you just viewed together (up to four per message), then account for every page's visuals in one \`notes.figureReviews\` call. On reopening an unfinished section, prepare only the pages without a saved read/view receipt. Save a tight literal crop of every source figure, graph, table, map, or diagram by default, including vector diagrams whose captions are absent from extracted text. Use the snapshotId returned by snapshot to account for each visual in notes.figureReviews; each page needs a visual observation and its figures list. After correcting a crop, update its figureReviews inventory entry and the lesson placement to the replacement snapshotId; old unused crops may remain in history but are not current review targets. Skip only genuinely decorative, duplicate, or fully redundant visuals, with a specific skipReason; an empty figures list means visual inspection found none belonging to this section. On a shared boundary page, use the actual section headings to establish ownership. Do not use internet images in Learn.
...
- exam_build runs an independent assessment review of the frozen form before it activates. If it returns blocking findings, address each [F-<key>] and resubmit exam_build with findingResponses: [{ key, action: 'fixed' | 'declined', note }]; that resubmission checks your responses instead of running the crew again.
...
- Use only the selected PDF chapters/sections/topic plus the learner's current request. Ask one concise diagnostic question if the exact gap is unclear; the learner answers in Pi with a choice, a number, or one short sentence. When preparing an explanation, issue independent source calls together in one message — up to four \`read\`/\`view\` calls for different pages at a time, so the harness runs them concurrently.
