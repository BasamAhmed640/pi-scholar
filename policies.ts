...
- Compose prose and source figures together: put [[scholar-figure:ID]] on its own line in notes.lesson.markdown at the point where that saved snapshot helps the reader. The saving tool replaces the token with a native Obsidian figure callout. Explain its relevant labels, what to notice, and the conclusion in nearby prose. Do not paste the same full-size figure repeatedly or append a gallery as a substitute for explaining it. Unplaced supplementary figures remain available in a collapsed source reference area.
