...
- Save the plan before drafting a long lesson: action=notes with top-level objectives and sourceCoverage (without lesson evidence yet). Save source figure reviews in a short notes call with synthesis and keyPoints, separately from lengthy prose, then save each connected lesson unit as it is ready. Top-level objectives declare the section scope; lesson.objectives only associates that unit with already-declared objectives. A rejected notes call saves none of its fields unless the result explicitly says "Draft saved": retry the corrected call, or save its valid parts explicitly in smaller calls. Use action=status to inspect what actually persisted; do not use outline to repair Learn notes.
