...
export const ScholarParams = Type.Object({
  ...
  objectives: Type.Optional(Type.Array(Type.String(), { description: "With Learn notes, declare the section's full source-grounded objective list here before saving lesson units. lesson.objectives references this list; it does not declare it. Omit after declaration to preserve the existing list; do not send an empty array." })),
  coveredObjectives: Type.Optional(Type.Array(Type.String())),
  requiredChecks: Type.Optional(Type.Array(CheckKindSchema)),
  synthesis: Type.Optional(Type.String({ description: "Concise source-grounded recap. A standalone recap save requires at least 40 characters and top-level keyPoints. For initial planning, save objectives plus sourceCoverage instead." })),
