...
assert.equal(await readFile(twoNotePath, "utf8"), twoBefore, "an unrelated book's note must still refresh");
...
const reported = [];
await projection.renderScholarWorkspace(config, [bookOne, bookTwo], (warning) => reported.push(warning));
const rebuilt = await readFile(sectionPath, "utf8");
assert.equal(reported.filter((warning) => warning.path === sectionPath).length, 1, "exactly one warning per damaged note");
assert(rebuilt.includes(QUARANTINE_START) && rebuilt.includes(TAIL), "the warning's claim of a rebuild is true");
const quiet = [];
await projection.renderScholarWorkspace(config, [bookOne, bookTwo], (warning) => quiet.push(warning));
assert.equal(quiet.length, 0, "a recovered note stops warning");
assert.equal(await readFile(sectionPath, "utf8"), rebuilt, "the recovered note is stable");
passed("marker damage is reported once, by the writer, and only when a rebuild actually happened");
